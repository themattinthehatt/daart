...
def run_main(hparams, *args):
    ...
    for expt_id in hparams['expt_ids']:
        ...
        # DLC markers
        markers_file = os.path.join(hparams['data_dir'], 'labels', expt_id + '_labeled.h5')
        # heuristic labels
    ...
    plot_training_curves(
        os.path.join(model_save_path, 'metrics.csv'), dtype='train', expt_ids=hparams['expt_ids'],
        save_file=os.path.join(model_save_path, 'train_curves'), format='png')
    plot_training_curves(
        os.path.join(model_save_path, 'metrics.csv'), dtype='val', expt_ids=hparams['expt_ids'],
